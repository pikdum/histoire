defmodule Histoire.TVDB.Lookup do
  use Ash.Resource,
    otp_app: :histoire,
  ...
  code_interface do
    ...
    define :get_movie, args: [:tvdb_id]
    define :get_series_by_slug, args: [:slug]
  end

  actions do
    action :search_titles, {:array, :map} do
      description "Search TVDB for series and movie candidates. Try alternate titles as needed."
      ...
      run fn input, _context ->
        with {:ok, rows} <- Histoire.TVDB.Client.search(input.arguments.query) do
          {:ok,
           rows
           |> Enum.filter(&(&1["type"] in ["series", "movie"]))
           |> Enum.map(&search_result/1)}
        end
      end
    end

    action :get_movie, :map do
      description "Fetch detailed TVDB evidence for one numeric movie ID."

      argument :tvdb_id, :integer do
        allow_nil? false
        public? true
        description "The numeric TVDB movie ID from search or a /movies/ URL"
      end

      run fn input, _context ->
        with {:ok, row} <- Histoire.TVDB.Client.fetch_movie(input.arguments.tvdb_id) do
          {:ok, movie_result(row)}
        end
      end
    end
    ...
    action :get_series, :map do
      ...
      run fn input, _context ->
        with {:ok, row} <- Histoire.TVDB.Client.fetch_series(input.arguments.tvdb_id) do
    ...
    action :search_web, {:array, :map} do
      ...
      run fn input, _context ->
        Histoire.TVDB.WebSearch.search(input.arguments.query)
      end
    end
    ...
    action :get_series_by_slug, :map do
      ...
      run fn input, _context ->
        ...
        if Regex.match?(~r/\A[a-z0-9][a-z0-9-]*\z/, slug) do
          with {:ok, %{"id" => id}} <- Histoire.TVDB.Client.fetch_series_by_slug(slug),
               {:ok, row} <- Histoire.TVDB.Client.fetch_series(id) do
            {:ok, series_result(row)}
          end
        else
  ...
  defp movie_result(row) do
    ...
    %{
      ...
      "slug" => row["slug"],
      "year" => row["year"],
      "first_released" => get_in(row, ["first_release", "date"]),
